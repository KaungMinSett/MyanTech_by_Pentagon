...
import { Package2, MapPin, Building2, ChevronLeft, ChevronRight } from 'lucide-react';
import { jsonData } from '../../mocks/warehouse/inventory-list-data';
...
function InventoryList() {
  ...
  const getProductDetails = (productId) => {
    ...
    const brand = jsonData.brands.find(b => b.id === product?.brand_id);
    const category = jsonData.categories.find(c => c.id === product?.category_id);
    return { product, brand, category };
  };

  const getWarehouseName = (warehouseId) => {
    return jsonData.warehouses.find(w => w.id === warehouseId)?.name;
  };

  const filteredInventory = jsonData.inventory.filter(item => {
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <select
                value={selectedBrand}
                onChange={(e) => {
                  setSelectedBrand(e.target.value === 'all' ? 'all' : Number(e.target.value));
                  setCurrentPage(1);
                }}
                className="block w-40 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
              >
                <option value="all">All Brands</option>
                {jsonData.brands.map(brand => (
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                ...
                {jsonData.warehouses.map(warehouse => (
                    ...
                    <tr key={`${item.product_id}-${item.warehouse_id}`} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          ...
                          <div className="text-sm text-gray-900">{getWarehouseName(item.warehouse_id)}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
